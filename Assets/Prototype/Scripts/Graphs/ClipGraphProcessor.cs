...
{
    public class ClipGraphProcessor : BaseGraphProcessor
    {
        ...
        public ClipGraphProcessor(BaseGraph graph) : base(graph) {}
				...
				if(node is IConditionalNode && !skipConditionalHandling.Contains(node))
				{
					...
					if(nodeDependenciesGathered.Contains(node))
					{
						...
						switch(node)
						{
							...
							case DelayedNode delayedNode:
				
								delayedNode.onProcessFinished += (waitedNode) =>
								{
									Stack<BaseNode> nodes = new Stack<BaseNode>();
									foreach(var n in waitedNode.GetExecutedNodes())
										nodes.Push(n);
									WaitedRun(nodes);
									delayedNode.onProcessFinished = null;
								};
								break;
