...
#pragma warning restore 0414, 0067, 0649
        ...
        private void Awake()
        {
            ...
            // !! START ADDITIONS !!
            var serialized = PlayerPrefs.GetString("QuantumConsoleHistory", "");
            var commands = serialized.Split('\n', StringSplitOptions.RemoveEmptyEntries);
            for (var i = 0; i < commands.Length; i++) {
                commands[i] = commands[i].Trim();
            }
            _previousCommands.AddRange(commands);
            // !! END ADDITIONS !!
        }

        // !! START ADDITIONS !!
        private void OnDestroy() {
            var minIndex = Mathf.Max(0, _previousCommands.Count - 20);
            var maxIndex = _previousCommands.Count;
            var sb = new StringBuilder();
            for (var i = minIndex; i < maxIndex; i++) {
                sb.Append(_previousCommands[i]);
                sb.Append('\n');
            }
            PlayerPrefs.SetString("QuantumConsoleHistory", sb.ToString());
        }
        // !! END ADDITIONS !!
